...
.controller('SocketCtrl', function ($log, $scope, chatSocket, messageFormatter, nickName) {
  ...
  $scope.sendMessage = function() {
    ...
    if($scope.messageLog)
      $scope.messageLog = $scope.messageLog + messageFormatter(new Date(), $scope.nickName, $scope.message );
    else
      $scope.messageLog = messageFormatter(new Date(), $scope.nickName, $scope.message );
  ...
  $scope.setEvents = function(){
    ...
    chatSocket.on('message', function(data){
        ...
        $scope.messageLog = $scope.messageLog + messageFormatter(new Date(), "Guest", data.payload);
      });
    });
  }
  $scope.setEvents();
